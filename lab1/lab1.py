...
identity_rule = IF( OR('female (?x)', 'male (?x)'), THEN('same-identity (?x) (?x)'))
brother_rule = IF( AND('parent (?y) (?x)', 'parent (?y) (?z)', 'male (?x)',
NOT('same-identity (?x) (?z)')), THEN('brother (?x) (?z)'))
sister_rule = IF( AND('parent (?y) (?x)', 'parent (?y) (?z)', 'female (?x)',
...
cousin_rule = IF( AND('sibling (?x) (?y)', 'parent (?x) (?z)',
'parent (?y) (?m)', NOT('same-identity (?z) (?m)')), THEN('cousin (?z) (?m)'))
grandparent_rule = IF( AND('parent (?x) (?y)', 'parent (?y) (?z)'), THEN('grandparent (?x) (?z)'))
grandchild_rule = IF('grandparent (?x) (?z)', THEN('grandchild (?z) (?x)'))

family_rules = [identity_rule, brother_rule, sister_rule, mother_rule,
...
WHAT_I_FOUND_INTERESTING = 'family relations'
